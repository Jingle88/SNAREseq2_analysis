...
# 定义输出文件
OUTPUT_BAM="${OUTPUT_DIR}/${SAMPLE}_aligned_sorted.bam"
...
# 运行 minimap2，仅比对 R1（R2 为 linker/technical sequence，不参与 genome alignment）
# 兼容 ALIGNER_PATH 为 minimap2 所在目录或 minimap2 可执行文件本身的情况
if [[ -d "$ALIGNER_PATH" ]]; then
    MINIMAP2_BIN="${ALIGNER_PATH%/}/minimap2"
else
    MINIMAP2_BIN="$ALIGNER_PATH"
fi

# 检查 minimap2 是否存在
if [[ ! -x "$MINIMAP2_BIN" ]]; then
    echo "Error: minimap2 executable not found or not executable: $MINIMAP2_BIN"
    exit 1
fi

# 检查 samtools 是否存在，因为这里需要直接 sort 成 BAM
if ! command -v samtools &> /dev/null; then
    echo "Error: samtools not found in PATH"
    exit 1
fi

"$MINIMAP2_BIN" -ax sr -t "$THREADS" "$REFERENCE" "$R1_FILE" | \
  samtools sort -@ "$THREADS" -T "$TMP_FOLDER/${SAMPLE}.tmp" -o "$OUTPUT_BAM" -
